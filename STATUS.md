...
| Item | Status | Notes |
| --- | --- | --- |
| Committed locally | ✅ Done | `main` tracking `origin/main`; note: **uncommitted WIP present** (see §2) |
...
| Status tracker committed | ✅ Done | This file — commit `31c327d` |

## 2. Working tree — uncommitted local WIP (not ours)

Observed 2026-09-07, **left untouched** — appears to be in-progress work by the
user (sound feature?):

- Modified: `apps/web/package.json`,
  `components/{dialogs,export-menu,file-tree,history-dialog,share-dialog,theme-toggle,workspace}.tsx`,
  `pnpm-lock.yaml`, `vitest.config.ts`
- Untracked: `apps/web/src/components/sound-toggle.tsx`, `apps/web/src/lib/sound.ts`

> ⚠️ Before any further commit/push, decide whether to commit this WIP or keep
> it out of `main`.
...
## 4. Browser E2E (`node cdp-e2e.mjs`, headless Chrome on :3111)
...
- [ ] Rerun focused wiki-link probe (login → open doc → type `[[x]]` → check pill)
      to classify item §4.2 as app bug or test artifact
...
- [ ] Decide what to do with the sound-feature WIP (§2)
- [ ] Delete scratch state in `/tmp` when finished (`markdocs-ui`,
      `markdocs-*-profile`, logs)

## 6. Scratch state (not part of the repo)
